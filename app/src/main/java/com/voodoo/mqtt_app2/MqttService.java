...
public class MqttService extends Service implements MqttCallback
{
    ...
    private static final String MQTT_BROKER = "m20.cloudmqtt.com"; // Broker URL or IP Address
    private static final int MQTT_PORT = 13363; // Broker Port
    private static final String MQTT_USER = "hhxlerts";
    private static final String MQTT_USER_PASSWORD = "JC-QW1mRKu5W";
    ...
    private static final String MQTT_PUBLISH_TOPIC = "cfg"; // Topic format for KeepAlives
    public static  String     MQTT_PUBLISH_MESSAGE = "000"; // Keep Alive message to send
    private static final int MQTT_PUBLISH_QOS = MQTT_QOS_0; // Default Keepalive QOS
    ...
    private static final String ACTION_PUBLISH = DEBUG_TAG + ".PUBLISH"; // Action to publish
    ...
    private MqttTopic mPublishTopic;
    ...
    public static void actionStart(Context ctx) {
            Log.i(DEBUG_TAG,"actionStart action of " + ctx.toString());
    ...
    public void onCreate() {
            ...
            mOpts.setPassword(MQTT_USER_PASSWORD.toCharArray());
            ...
            mConnHandler.post(new Runnable() {
                            ...
                            public void run() {
                                    try {
                                            ...
                                            mClient.subscribe("stt", 0);
                                            //mClient.subscribe("alrm", 0);
    ...
    public synchronized MqttDeliveryToken publishTopic()
            throws MqttConnectivityException, MqttPersistenceException, MqttException {
        if(!isConnected())
            throw new MqttConnectivityException();

        if(mPublishTopic == null) {
//            mPublishTopic = mClient.getTopic(
//                    String.format(Locale.US, MQTT_PUBLISH_TOPIC_FORAMT, mDeviceId));
            mPublishTopic = mClient.getTopic(MQTT_PUBLISH_TOPIC);
        }

        Log.i(DEBUG_TAG,"Publishing to " + MQTT_BROKER);

        MqttMessage message = new MqttMessage(MQTT_PUBLISH_MESSAGE.getBytes());
        message.setQos(MQTT_PUBLISH_QOS);

        return mPublishTopic.publish(message);
    }
    /**
    ...
    throws Exception {
        ...
        char[] str = new String(message.getPayload()).toCharArray();
        if(str[0] == 'O')
        {
            if(str[1] == 'A' && mp == null)
            {
        ...
        views.setTextViewText(R.id.textView, "topic: " + topic.getName() + " data: " + new String(message.getPayload()));

        Intent intent = new Intent(MainActivity.BROADCAST_ACTION);
        intent.putExtra(MainActivity.PARAM_TOPIC, topic.getName());
        intent.putExtra(MainActivity.PARAM_DATA, new String(message.getPayload()));
...
//            profileImage.setImageBitmap(Bitmap.createScaledBitmap(bitmap , 64, 64, false));
            ...
            builder.setContentIntent(contentIntent)
                    ...
                    // большая картинка
                    .setLargeIcon(BitmapFactory.decodeResource(res, R.drawable.intruder_small))
                    ...
                    .setContentText(currentDateandTime); // Текст уведомления
